/**
 * The character model — the player's raw choices for a build.
 *
 * Everything derived (final scores, HP, AC, saves) is computed from this by the
 * engine (`engine.ts`); this state stays minimal and is what we
 * serialize/deserialize. It lives in `@pathway/core` so the bot and web agree
 * on the shape a character takes.
 */
...
export interface BuilderState {
  ...
  /** Optional character portrait, stored as a small data URL. */
  portrait?: string;
  ...
  /** Character-scoped option toggles (variant rules etc.); see OPT. */
