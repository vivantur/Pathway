...
import { pendingFeatChoices } from './rules';
import { plainText } from './contentText';

/**
 * Consolidated prompts for feats whose ChoiceSet drives a proficiency grant
 * (Canny Acumen → a save/Perception, Natural Skill → two skills, …). Renders
        ...
        {pending.map(({ feat, prompts }) => (
          <div key={feat.id} className="flex flex-col gap-2 rounded-md border border-parchment/10 p-3">
            ...
            <div className="flex flex-wrap gap-3">
              {prompts.map((prompt) => {
                const current = state.featChoices?.[feat.id]?.[prompt.flag] ?? '';
                return (
