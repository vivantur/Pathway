// Animal companion catalog + derived-stat engine (+ familiar/eidolon data).
//
// SOURCE OF TRUTH NOTE: this is a verbatim copy of
// `packages/core/src/companion.ts` (which carries the vitest coverage). It is
// vendored here because Vercel builds apps/web with Root Directory = apps/web
// and prunes sibling packages, so the @pathway/core workspace import is not
// resolvable at build time (same reason `builder/data/proficiency.ts` is
// vendored). Keep the two files identical below this header.
