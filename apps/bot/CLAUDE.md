...
So: **the bot's tooling (vitest, eslint, …) lives in the ROOT `package.json`'s
devDependencies.** npm hoists it, so `npm --workspace apps/bot run test` still works
locally and in CI. `apps/bot/package.json` declares production dependencies only,
which keeps it in sync with `apps/bot/package-lock.json`.
...
also let the root `overrides` (e.g. the `undici` security pin) reach production.
That build needs `npm ci --include=dev`, since core's `prepare` runs `tsc` and
Railway sets npm's `production` config. See the root `CLAUDE.md`.
