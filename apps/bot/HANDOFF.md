...
3. ~~**Post-cutover cleanup**~~ — Done (2026-07-08): combat is consolidated on `rules/combatV2/`. The legacy engine (`commands/encounters.js`, `rules/combatAutomation.js`), both legacy summary renderers, and the dead scaffolds are deleted. Along the way the consolidation fixed live bugs: `/cast` targeting, `/companion use`/`roll`, and `/m attack use` were bound to the unreachable legacy store; four registered `/init` subcommands errored; v2 lacked the crit→Dying 2 rule and the hero-point reroll/escape-death buttons. All are v2-native now, covered by `test/combatV2.test.js`.
4. ~~**Add a test suite**~~ — Done (2026-07-08): 146 Vitest tests in `test/` cover the pure rules layer (dice/degree-of-success/MAP, proficiency math, /roll parser, conditions, spell heightening, the dying/wounded/recovery engine, formatters, variables). `npm test` runs them; see the Testing section in `CLAUDE.md`. Next: extend coverage into `rules/combatV2/` during the combat-engine consolidation.
