...
const charCommand = new SlashCommandBuilder()
  ...
  .addSubcommand(s => s.setName('pastemsg').setDescription('Import from a JSON message you just posted in this channel (mobile-friendly).'))
  .addSubcommand(s => s.setName('pastemsgupdate').setDescription('Update from a JSON message you just posted (preserves HP/XP/overlay).'))
  .addSubcommand(s => s.setName('pdf').setDescription('Import from a Pathbuilder PDF export.')
    .addAttachmentOption(o => o.setName('file').setDescription('Pathbuilder PDF file').setRequired(true)))
  ...
  .addSubcommand(s => s.setName('lore')
    ...
    .addStringOption(o => o.setName('character').setDescription('Character name').setRequired(false).setAutocomplete(true)))
  .addSubcommand(s => s.setName('money')
    .setDescription('Set coin counts on the character sheet (for /sheet display).')
    .addStringOption(o => o.setName('action').setDescription('set (default) or clear all overrides').setRequired(false).addChoices({ name: 'Set', value: 'set' }, { name: 'Clear', value: 'clear' }))
...
const mattackCommand = new SlashCommandBuilder()
  ...
  .addStringOption(o => o.setName('attacker').setDescription('Combatant doing the attacking').setRequired(true))
  .addStringOption(o => o.setName('name').setDescription('Attack name (shown in embed title)').setRequired(true))
  .addIntegerOption(o => o.setName('bonus').setDescription('Attack bonus (e.g. 8 for +8)').setRequired(true))
  .addStringOption(o => o.setName('damage').setDescription('Damage expression (e.g. 2d6+4)').setRequired(true))
